...
class RunLs
  def initialize(pathname, width)
    @pathname = pathname.glob('*').sort
    @filename = @pathname.map(&:basename).map(&:to_s) #{ |f| File.basename(f) }.to_s
  end

  def max_filename_count
    @filename.map(&:size).max
  end

  def column_count(width)
    width / (max_filename_count + 1)
  end

  def row_count
    (@filename.count.to_f / column_count(width)).ceil
  end

  def transpose_filenames
    transpose(@filename.each_slice(row_count).to_a)
  end

  def transpose(nested_filenames)
    nested_filenames[0].zip(*nested_filenames[1..-1])
  end

  def format_table(max_filename_count)
    @filename.map do |row_files|
      row_files.map { |filename| filename.to_s.ljust(max_filename_count + 1) }.join.rstrip
    end
  end
